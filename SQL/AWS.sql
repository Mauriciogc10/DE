Problem Statement:
write a query to find a number of active prime member at the end of 2020 in each marketplace.
Assume that one customer can be only one marketplace
...
with cte as( select *,
row_number() over(partition by customer_id order by event_date desc) rnk
from subscription_history where event_date‹= '2020-12-31' )
select *,DATEADD(month, subscription_period, event_date) as till_date
from cte
where rnk=1 and DATEADD (month, subscription_period, event_date)>='2020-12-31' and event!='C'